# 1. confidence_score_solved_final.ipynb:
...
6. it produces all the json files needed for statistics, xcomet error spans:
   ...
   3. all_entries_with_opcodes_translator.json - find in **statistics_translator.ipynb**
   ...
   7. relevant_words_in_pet_translator_without_mapping.json - find in **statistics_translator.ipynb**
...
# 2. changepredictfinal.ipynb:

This file is showing how to use custom xcomet class and there are codes that shows my implementation of xcomet works. does not do any things related to tables. so can skip this for english to czech experiment, just use it to recheck it that it still works for the newer models, if it generates the same output or not.

# 3. compare_xcomet_default_and_our_version.ipynb:

It compares the spans we get from the default xcomet version with the spans we get from our updated xcomet version. It loads the dataset **with open("output_all_spans_community.json", "r", encoding="utf-8") as f:** that we got the **confidence_score_solved_final.ipynb** and it has updated xcomet spans, and it loads the file which was generated by default xcomet version from here:

xcometdefault = "/storage/brno2/home/rahmang/xcomet/cometdefault/COMET/output_all_spans_community_defaultcomet.json"

and both files contains exact 659 spans, only difference is that default xcomet has subwords and our xcomet has word level spans.

Later it uses difflib sequence matcher to show the similarity between two versions of the output, and it has 92% similarity.

# 4. df_with_spans_relevant_words.ipynb

It just loads this:

statistics_results_json = "/storage/brno2/home/rahmang/xcomet/arafat_comet/COMET_GR/all_entries.json"

and check if there are relevant words in the json file or not. However, **all_entries.json** was replaced by **with open("all_entries_bug_fixed_with_opcodes_community.json", "w", encoding="utf-8") as f:
    json.dump(statistics_results_json, f, ensure_ascii=False, indent=4)  # Format for readability** in the **confidence_score_solved_final.ipynb** file.

# 5. statistics_translator.ipynb

Its the **confidence_score_solved_final.ipynb** for the translator dataset. can find these outputs:

1. "output_all_spans_translator.json"
2. "all_entries_with_opcodes_translator.json"
3. "relevant_words_in_pet_translator_without_mapping.json"

# 6. tp_fp_using_difflib_community.ipynb:

It uses ""output_all_spans_community.json"  and ""relevant_words_in_pet_community_without_mapping.json"" from **confidence_score_solved_final.ipynb** and calculates the tp_fp using difflib sequence matcher.
